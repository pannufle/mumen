...
def get_translations(lemma, target_lang_iso_1, config):
    """Get WordNet translations.

    Get translations of word (with pos) from source to target language
    """
    ...
    synsets = wn.synsets(word, pos=const.MEN2WN_POS[pos],
                         lang=source_lang_iso_2)
    if synsets:
    ...
    for synset in synsets:
        ...
        if wn_target_lemmas:
            logger.debug('Corresponding target lemmas: {}'
                         .format(wn_target_lemmas))
        else:
